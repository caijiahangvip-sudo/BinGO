...
- Do not assume the OpenClaw agent's own model or API key will be reused by Bingo.
- Bingo classroom generation uses Bingo server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Bingo server-side config files may control provider selection and defaults.
...
If no `accessCode` in config, ask the user how they want to use Bingo:

1. **Use hosted Bingo** (recommended for quick start) — Requires an access code from open.maic.chat. No local setup needed.
...
Use this when the user has not installed Bingo yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Bingo

After the user has chosen a startup mode and configured keys, start Bingo using the chosen method, then verify the service with `GET {url}/api/health`.
