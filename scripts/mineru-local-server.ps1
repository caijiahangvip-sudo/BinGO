param(
  [string]$InstallDir = "$PSScriptRoot\..\dev\MinerU",
  [int]$Port = 50002,
...
if ($env:BINGO_LOCAL_MODEL_RUNTIME -and $env:BINGO_LOCAL_MODEL_RUNTIME.ToLowerInvariant() -eq "windows") {
  throw "Windows-native MinerU startup has been disabled by default. Clear BINGO_LOCAL_MODEL_RUNTIME or use the WSL ROCm launcher."
}
