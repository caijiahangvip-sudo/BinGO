...
- **2026-03-26** — [v0.1.0 发布！](https://github.com/THU-MAIC/Bingo/releases/tag/v0.1.0) 讨论语音、沉浸模式、键盘快捷键、白板增强、新 provider 等。查看[更新日志](CHANGELOG.md)。

## 📖 项目简介

**Bingo**（Open Multi-Agent Interactive Classroom）是一个开源的 AI 互动课堂平台，能够将任何主题或文档转化为丰富的互动学习体验。基于多智能体协作引擎，它可以自动生成演示幻灯片、测验、交互式模拟实验和项目制学习活动——由 AI 教师和 AI 同学进行语音讲解、白板绘图，并与你展开实时讨论。
...
向量 / embedding 服务会在设置中的 **向量模型** 独立配置。内置项包括本地新华字典向量服务，以及 OpenAI、Qwen、SiliconFlow embedding。
...
>
> 如果希望 Bingo 服务端默认走 Gemini，还需要额外设置 `DEFAULT_MODEL=google:gemini-3-flash-preview`。
>
...
这会同时启动 Bingo 应用、本地 CosyVoice、SenseVoice、MinerU 和 embedding 服务。打开 **http://localhost:4000** 开始学习！

本地模型服务默认使用 WSL2 ROCm。若 ROCm PyTorch 看不到 HIP GPU，会直接报错并输出诊断，不会静默回落到 CPU。可选配置：`BINGO_WSL_DISTRO`、`BINGO_WSL_RUNTIME_ROOT`、`BINGO_WSL_PYTHON`、`BINGO_PIP_INDEX_URL`。内置 ROCm wheel 需要 WSL 内的 Python 3.10 或 3.12。
...
描述你想学习的内容，或附上参考材料。Bingo 的两阶段流水线自动完成剩余工作：
